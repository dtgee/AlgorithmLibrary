...
public class PalindromeGroup {
	 ...
	 *  so that all the palindromes are next to eachother.
	 * 
	 *  Before starting it's probably a good idea to
	 *  write a utility method for detecting of a
	 *  String is an palindrome.
	 * 
	 *  After we mark each string as palindrome or not,
	 *  perform either a bucket sort or a partition 
	 *  on the list to group by palindrome/non-palindrome.
	 *  
	...
	static String[] group_palindromes(String[] list) {
		/* 
		 * We will perform the grouping in a similar
		 * fashion to quicksort's partition. This is 
		 * inplace and O(nlogn) 
		 * 
