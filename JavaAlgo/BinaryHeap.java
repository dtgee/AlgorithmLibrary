...
//Purpose: Fill up with a bunch of HeapNodes, which are simply objects which hold
//vertices and Dv values for those vertices. Compute the "most worthy" Dv value, and
//return that value along with its corresponding vertex.

//ArrayList based. (Minimum) Heap. Handles compute by distance/time based off of a
//constructor variable. Root is at index 1. Children are at positions 2i and 2i + 1,
//grand children follow the same pattern. 
public class BinaryHeap
{
	...
	public BinaryHeap()
	{
		...
		//The index zero of my list will have a dead weight object so we can start at 1.
		this.mainList.add(null);
		//NOTE: This size is the size of the heap and independent of the array list,
		//the array list will always be +1 over the heap because of the dead weight. 
		this.size = 0;
	}
	...
	//INSERTION:
	//Make sure the data-structure still follows heap rules, parents are always smaller
	//than the children, siblings don't need to be in any order. 
	//If an insertion violates rules, keep swapping until it works.
	public void insert(Vertex vertex, double key)
	{
		mainList.add(new HeapNode(vertex, key));
		//Because of the dead weight, we need to subtract one to the array list size
		//to get the accurate insert indices. 
		...
		while(followIndex > 1)
		{
			...
			{
				//This value means nothing, it's purpose is so we can leave this loop.
				followIndex = 0; 		
			}
		}
		size++;
	...
	//DELETION:
	//Remove the node (because it's what we want anyways) and swap the empty spot with
	//the last node of the list (it's always complete so w/e last item of the array list).
	//Afterwards, if the structure is wrong, keep swapping with the smallest child until
	//the structure works.
	public Vertex removeMin()
	{
		...
		//Set the root of the tree to be the very last node of the complete heap.
		...
		while(followIndex < mainList.size() - 1)
		{
			...
			if(mainList.size() <= left) // Size inflated by 1 because deadweight, so we use <= and not <
			{ 
				followIndex = mainList.size() - 1;
			}
				
			//If the right child doesn't exist and the left is smaller, swap left to root.
			...
			//If the right/left children actually exist, replace with the smaller. 
			else 
			{   
				//If the left child is smaller than the right child. (Or equal)
				if((mainList.get(left).key < mainList.get(right).key || 
						mainList.get(left).key == mainList.get(right).key) && 
...
class Edge 
{
	...
	//A special field designed so I can identify out bound and in bound edges for each
	//vertex. 
	private boolean isSuccessor;
		
	public Edge(int startVertex, int adjacentVertex, double distance, double speed)
	{
	...
	//This method is more beefy because we need to compute the actual time taken
...
//	v is known. Initially, kv is false for all vertices.
	private boolean k;
//	dv is the length of the shortest known path from the start vertex to vertex 
