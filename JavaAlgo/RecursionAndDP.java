...
public class RecursionAndDP {
	...
	/*
	 * The following will be a list of methods which
	 * are my solutions to problems on chapter 9 on
	 * Cracking the Coding Interview
	 */
	
	/*
	 * A child is running up a staircase with 
	 * n steps. He can go 1, 2, or 3 steps at 
	 * a time. Count how many possible ways 
	 * he can run up the stairs.
	 */
	 public static int naiveSteps(int n) {
		 ...
		 return naiveSteps(n-3) + naiveSteps(n-2) + 
				 naiveSteps(n-1);
	 }
	 ...
	 public static int smartSteps(int n, int[] memo) {
		 ...
		 if (memo[n] > -1) {}
			 ...
			 if (n < 3) {
				 memo[n] = smartSteps(n-2, memo) + 
						 smartSteps(n-1, memo);
			 }
			 else {
				 memo[n] = smartSteps(n-3, memo) + 
						 smartSteps(n-2, memo) + 
						 smartSteps(n-1, memo);
			 }
		 }
