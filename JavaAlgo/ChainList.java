...
public class ChainList
{
	...
	// masterFileName line: ###_###_##### (area code - phone prefix - zip code (size varies))
	public ChainList(AbstractMapKey mapKeyToUse, String masterFileName) throws IOException 
	{
	...
	public void fakeInitializer(AbstractMapKey mapKeyToUse, String masterFileName) throws FileNotFoundException
	{
		//Nothing
	}
	...
	private void buildList(String masterFileName)
	{
		while(mainScanner.hasNextLine())
		{
			...
			disector.close();
			this.insert(new ZipcodeGroup(zipCode, new PhoneGroup(areaCode, phonePrefix))); 
		}
	}
	...
	{
		Chain altarThisChain = chainList.get(mapKey.computeMapKey(zipGroup.getZipcode()));
		altarThisChain.insert(zipGroup);
		chainList.set(mapKey.computeMapKey(zipGroup.getZipcode()), altarThisChain);
	}
	...
	{ 
		return chainList.get(mapKey.computeMapKey(key)).find(key).getPhoneGroups();
	}
    ...
    // We use our own linked-list and node structures for speed: Java's LinkedList
	...
	private class Chain
	{
		...
		public void insert(ZipcodeGroup zipGroup)
		{
			...
			//If the chain is empty, insert the node and set current, starting positions.
			if(startOfChain == null)
			{
				startOfChain = new Node(zipGroup, null);
			}
			//My order algorithm will be stored so bigger ZIP's in the front of the list.
			else
			{
				Node toBeInserted = new Node(zipGroup, null);
				//If it is the case that we have an existing Zip code, no new nodes are
				...
				while(!zipEntryPointFound)
				{
					...
					{
						current.addPhoneGroup(zipGroup.getPhoneGroupList().get(0));
						zipEntryPointFound = true;
					}
		...
		public Node find(int keyToFind) throws NoSuchZipException
		{
			...
			while(current != null)
			{
				//Because my chains are sorted in ascending order, if our zip is smaller
				//than the current value, we know our zip wont be present in the list.
		...
		public String toString()
		{
			...
			while(current != null)
			{
				...
				{   
					smallBuffer.append("Area: " + current.getPhoneGroups().get(i).getAreaCode()
							+ " Prefix: " + current.getPhoneGroups().get(i).getPrefix() + " || ");
				}
				
				returnBuffer.append("     Key: " + current.getZipcode() + "    Values: " + 
